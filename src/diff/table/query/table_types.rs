...
impl IncludedExcludedTables {
    pub fn new(
        ...
        Self {
            included_tables: include_tables.to_vec(),
            excluded_tables: exclude_tables.to_vec(),
        }
    }
...
mod tests {
    ...
    fn test_when_included_tables_and_excluded_tables_are_both_not_empty() {
        let included_tables = vec!["table1".to_string()];
        let excluded_tables = vec!["table2".to_string()];
